...
describe("processor", () => {
  ...
  describe("#exec", () => {
    ...
    it("file update", () => {
      fs.writeFileSync(
        "password.txt",
        "mihrdat:{SHA}NSjjR5VzkksaQNgdEKs0MqPtStI=\n",
        "UTF-8"
      );
      const program = {
        batch: true,
    ...
    it("file delete", () => {
      fs.writeFileSync(
        "password.txt",
        "urmia:{SHA}NSjjR5VzkksaQNgdEKs0MqPtStI=\n",
        "UTF-8"
      );
    ...
    it("file add", () => {
      ...
      const fileData = fs.readFileSync("password.txt", "UTF-8");
      expect(fileData).to.equal(
        `${initData}tigran:{SHA}1cYTTawbHcKifj2Yn6MC8bDq7Dc=\n`
      );
    });
    ...
    it("file add, not existing", done => {
      ...
      console.error = function() {
        ...
        expect(arguments[0]).to.equal(
          "Cannot modify file password.txt; use '-c' to create it."
        );
        done();
      };
  ...
  describe("#validate", () => {
    ...
    it("username, password", () => {
      // Source.
      const valid = processor.validate({
        nofile: true,
        batch: true,
        args: ["kiara", "superPass"]
      });
